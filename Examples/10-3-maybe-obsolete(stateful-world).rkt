...
;; initial-world : -> World
;; RETURNS: a world with a wall and a ball that knows about the wall.
...
(define World%
  (class* object% (StatefulWorld<%>)
    ...
    ;; (*) -> Void
    ;; Distributes fn to each widget or stateful widget, and updates
    ;; the world appropriately.
    ;; (*) fn must satisfy both of the following contracts
    ;; (Widget -> Widget)    [first use of fn, with map]
    ;; AND (SWidget -> Void) [second use, with for-each]
